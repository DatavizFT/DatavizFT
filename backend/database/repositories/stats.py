...
from ...models.competence import CompetenceStats, StatistiqueCompetence
...
class StatsRepository:
    ...
    async def get_latest_stats(self) -> Optional[Dict[str, Any]]:
        ...
        """
        cursor = self.collection_stats.find({}).sort("date_analyse", DESCENDING).limit(1)
        results = await cursor.to_list(length=1)
        return results[0] if results else None
    
    async def get_stats_by_periode(self, periode: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def get_evolution_competence(
        ...
        """
        cursor = self.collection_stats.find({
            "competences_stats.competence": competence
        }).sort("date_analyse", DESCENDING).limit(nb_periodes)
    ...
    async def calculate_realtime_stats(self, jours: int = 30) -> Dict[str, Any]:
        ...
        for comp_stat in competences_stats:
            pourcentage = (comp_stat["nb_offres"] / total_offres) * 100 if total_offres > 0 else 0
    ...
    async def get_tendances_competences(self, seuil_croissance: float = 0.1) -> Dict[str, List[str]]:
        """
        ...
        cursor = self.collection_stats.find({}).sort("date_analyse", DESCENDING).limit(2)
        ...
        for competence, pourcentage_actuel in stats_actuelles.items():
            ...
            if pourcentage_precedent > 0:
                variation = (pourcentage_actuel - pourcentage_precedent) / pourcentage_precedent
    ...
    async def get_stats_geographiques(self, competence: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def cleanup_old_stats(self, nb_periodes_a_garder: int = 24) -> int:
        ...
        cursor = self.collection_stats.find({}).sort("date_analyse", DESCENDING).limit(nb_periodes_a_garder)
