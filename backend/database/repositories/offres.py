...
class OffresRepository:
    ...
    async def insert_offre(self, offre: OffreEmploiModel) -> Optional[str]:
        """
    ...
    async def get_offre_by_source_id(self, source_id: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def get_offres_recentes(self, jours: int = 7, limit: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        }).sort("date_creation", DESCENDING).limit(limit)
    ...
    async def get_offres_by_competence(
        ...
        """
        cursor = self.collection.find({
            "competences_extraites": competence.lower()
        }).sort("date_creation", DESCENDING).limit(limit)
    ...
    async def get_offres_by_localisation(
        ...
        cursor = self.collection.find(query).sort("date_creation", DESCENDING).limit(limit)
        return await cursor.to_list(length=limit)
