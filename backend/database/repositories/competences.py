...
from ...models.competence import CompetenceModel, CompetenceDetectee
...
class CompetencesRepository:
    ...
    async def insert_competence(self, competence: CompetenceModel) -> Optional[str]:
        """
    ...
    async def get_competence_by_nom(self, nom: str) -> Optional[Dict[str, Any]]:
        """
    ...
    async def get_competences_by_categorie(self, categorie: str) -> List[Dict[str, Any]]:
        """
        ...
        """
        cursor = self.collection.find({"categorie": categorie}).sort("popularite", DESCENDING)
        return await cursor.to_list(length=None)
    
    async def search_competences(self, terme: str, limit: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        }).sort("popularite", DESCENDING).limit(limit)
        
        return await cursor.to_list(length=limit)
    
    async def update_popularite_competence(self, nom: str, nouvelle_popularite: float) -> bool:
        """
    ...
    async def log_detection_competence(
        self, 
        offre_id: str, 
        competences_detectees: List[CompetenceDetectee]
    ) -> bool:
        """
    ...
    async def sync_competences_from_detections(self) -> Dict[str, int]:
        ...
        for stat in stats_detections:
            ...
            nouvelle_popularite = min(stat["confiance_moyenne"] * stat["nb_detections"] / 1000, 1.0)
            
            if await self.update_popularite_competence(competence_nom, nouvelle_popularite):
                updates_count += 1
