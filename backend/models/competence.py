...
class CompetenceModel(BaseModel):
    ...
    nom_normalise: Optional[str] = Field(None, description="Nom normalisé (auto-généré si non fourni)")
    categorie: CategorieCompetence
    synonymes: List[str] = Field(default_factory=list, description="Synonymes et variantes")
    description: Optional[str] = None
    
    # Métadonnées
    popularite: Optional[float] = Field(None, ge=0.0, le=1.0)
    difficulte: Optional[int] = Field(None, ge=1, le=5)
...
class CompetenceDetectee(BaseModel):
    ...
    confiance: float = Field(ge=0.0, le=1.0, description="Score de confiance de la détection")
    position: Optional[int] = Field(None, description="Position dans le texte")
...
class StatistiqueCompetence(BaseModel):
    ...
    # Tendances
    tendance: Optional[TendanceEvolution] = None
    croissance_annuelle: Optional[float] = None
...
class CompetenceStats(BaseModel):
    ...
    def get_competence_stats(self, competence: str) -> Optional[StatistiqueCompetence]:
    ...
    def get_top_n_competences(self, n: int = 10) -> List[StatistiqueCompetence]:
        ...
        return sorted(
            self.competences_stats, 
            key=lambda x: x.nb_offres, 
            reverse=True
        )[:n]
...
class EvolutionCompetence(BaseModel):
    ...
    evolution_points: List[Dict[str, Any]] = Field(
        ...
        description="Points d'évolution temporelle"
    )
    
    # Métriques calculées
    taux_croissance_mensuel: Optional[float] = None
    volatilite: Optional[float] = None
    saisonnalite: Optional[Dict[str, float]] = None
    
    # Prédictions
    prediction_6mois: Optional[Dict[str, Any]] = None
    confiance_prediction: Optional[float] = Field(None, ge=0.0, le=1.0)
    
    def ajouter_point_evolution(self, date: datetime, nb_offres: int, contexte: Dict = None):
