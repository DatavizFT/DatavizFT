...
def charger_referentiel_competences() -> Dict[str, List[str]]:
    ...
    try:
        ...
        raise FileNotFoundError(f"Fichier competences.json non trouvé dans {Path(__file__).parent}")
...
NB_COMPETENCES_TOTAL = sum(len(competences) for competences in COMPETENCES_REFERENTIEL.values())
