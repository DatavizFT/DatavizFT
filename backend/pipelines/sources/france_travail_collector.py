...
class FranceTravailCollector(BaseDataCollector):
    ...
    async def collect_and_save_jobs_optimized(  # noqa: C901
        ...
        try:
            logger.info(
                f"🚀 Collecte France Travail optimisée - Code ROME: {self.code_rome}"
            )
            ...
            logger.info(
                f"📊 {len(raw_jobs)} offres collectées depuis l'API France Travail"
            )

            # 2. Récupération des source_id existants pour déduplication
            source_ids_api = {str(job.get("id", "")) for job in raw_jobs}
            source_ids_existants = await self.offres_repo.get_existing_source_ids(
                list(source_ids_api)
            )

            # 3. Récupération des offres actives (sans date_cloture) pour gestion des clôtures
            offres_actives = await self.offres_repo.get_active_offers_by_source(
                self.source_name
            )
            ...
            if source_ids_a_cloturer:
                nb_clôturees = await self.offres_repo.close_offers(
                    list(source_ids_a_cloturer)
                )
            ...
            for raw_job in raw_jobs:
                ...
                texte_offre = (
                    f"{raw_job.get('intitule', '')} {raw_job.get('description', '')}"
                )
                competences_detectees = self.competence_analyzer.analyser_texte(
                    texte_offre
                )
    ...
    async def collect_raw_jobs(
        self, max_jobs: int | None = None, **_kwargs
    ) -> list[dict]:
        """
        ...
        try:
            ...
            offres = self.api_client.collecter_offres_par_code_rome(
                self.code_rome, max_offres=max_jobs
            )
    ...
    async def convert_job_to_model(self, raw_job: dict) -> dict:
        ...
        return {
            ...
            "entreprise": {
                ...
                "entrepriseAdaptee": raw_job.get("entreprise", {}).get(
                    "entrepriseAdaptee"
                ),
            },
            ...
            "contrat": {
                ...
                "dureeTravailLibelleConverti": raw_job.get(
