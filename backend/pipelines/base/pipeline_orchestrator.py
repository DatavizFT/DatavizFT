...
class PipelineOrchestrator:
    ...
    async def check_should_analyze(self, hours_limit: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            last_analysis = await self.db.pipeline_executions.find_one(
                {"pipeline_name": self.pipeline_name, "type": "analyse_competences"},
    ...
    async def check_should_generate_stats(
        self, hours_limit: int = 24
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            last_stats = await self.db.pipeline_executions.find_one(
                {"pipeline_name": self.pipeline_name, "type": "statistiques"},
                sort=[("derniere_execution", -1)],
            )
        ...
        **collect_kwargs,
        ...
        try:
            ...
            if should_collect:
                ...
                collection_results = (
                    await self.data_collector.collect_and_save_jobs_optimized(
            ...
            if should_analyze:
                logger.info(
                    f"🧠 Début analyse des compétences {self.data_collector.source_name}"
                )

                # Créer le processeur de compétences
                from .competence_processor import CompetenceProcessor

                competence_processor = CompetenceProcessor(
                    self.data_collector.source_name
                )
            ...
            if should_generate_stats and self.stats_generator:
                logger.info(
                    f"Début génération statistiques {self.stats_generator.source_name}"
                )
                ...
                logger.warning(
                    f"⚠️ StatsGenerator non configuré pour {self.pipeline_name}"
                )
                stats_results = {
                    "success": False,
