...
class CompetenceProcessor:
    ...
    async def analyze_jobs_competences(
        ...
        try:
            ...
            logger.info(f"📊 Analyse de {len(offres_a_analyser)} offres pour {self.source_name}")
            ...
            await self._enregistrer_execution_analyse(
                pipeline_name,
                len(offres_a_analyser),
                len(competences_par_offre)
            )
    ...
    def _construire_mapping_competences_offres(
        ...
        offres_pour_analyse: list[dict[str, Any]]
    ) -> dict[str, list[str]]:
        """
        ...
        for categorie, data_categorie in resultats.get("resultats_par_categorie", {}).items():
        ...
        logger.info(f"Mapping construit: {len(competences_par_offre)} offres avec compétences")
        return competences_par_offre
    
    async def _sauvegarder_competences_detectees(
        self, 
        competences_par_offre: dict[str, list[str]],
        pipeline_name: str
    ) -> int:
        """
    ...
    async def _mettre_a_jour_offres_avec_competences(
        ...
        for offre_id, competences_liste in competences_par_offre.items():
            ...
            competences_noms = competences_liste if isinstance(competences_liste, list) else []
    ...
    async def get_analysis_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            nb_offres_avec_competences = await self.offres_repo.collection.count_documents({
                "source": self.source_name,
            ...
            return {
                ...
                "pourcentage_traitees": round(
                    (nb_offres_traitees / nb_offres_total * 100) if nb_offres_total > 0 else 0, 2
                ),
                "pourcentage_avec_competences": round(
                    (nb_offres_avec_competences / nb_offres_total * 100) if nb_offres_total > 0 else 0, 2
                ),
                "derniere_analyse": (
