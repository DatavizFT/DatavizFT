...
class PipelineM1805:
    ...
    def collecter_offres(self, max_offres: int | None = None) -> list:
        """
    ...
    def sauvegarder_donnees(
        self, offres: list, resultats_analyse: dict[str, Any]
    ) -> dict[str, str]:
        """
    ...
    def executer_pipeline_complet(
        ...
        """
        print("🚀 EXÉCUTION DU PIPELINE COMPLET M1805")
        print("=" * 70)
        ...
        try:
            ...
            print(f"\n❌ ERREUR PIPELINE: {e}")
            return {"success": False, "error": str(e), "nb_offres": 0}
    ...
    def obtenir_statistiques_pipeline(self) -> dict[str, Any]:
        ...
        return {
            ...
            "nb_competences_total": sum(
                len(comp) for comp in self.competences_referentiel.values()
            ),
...
def run_pipelineFT(forcer_execution: bool = False) -> dict[str, Any]:
    ...
    if not forcer_execution:
        ...
        if not verification["doit_executer"]:
            ...
            print(
                f"📁 Dernier fichier: {os.path.basename(verification['dernier_fichier'])}"
            )
