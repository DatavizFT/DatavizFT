...
def collecter_toutes_offres():
    ...
    try:
        # Utilisation du nouveau client API intégré
        client = FranceTravailAPIClient()
        return client.collecter_offres_par_code_rome(CODE_ROME)
    ...
    for categorie, liste_competences in COMPETENCES_REFERENTIEL.items():
        ...
        for competence in liste_competences:
            ...
            for offre in offres:
                ...
                # Tester les patterns
                if any(
                    re.search(pattern, texte_offre, re.IGNORECASE)
    ...
    try:
        ...
        for categorie, competences in resultats_par_categorie.items():
            for comp in competences:
                if comp["nb_offres"] > 0:
                    toutes_competences.append({**comp, "categorie": categorie})
        ...
        donnees_finales = {
            ...
            "statistiques_globales": {
                ...
                "nb_competences_total": sum(
                    len(cat) for cat in resultats_par_categorie.values()
                ),
...
def run_pipeline():
    ...
    try:
        ...
        fichier_competences = sauvegarder_competences(
            resultats_competences, len(offres)
        )
        ...
        print("   • Analyse complète des compétences effectuée")
