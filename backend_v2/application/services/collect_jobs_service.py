...
class CollectJobsService:
    ...
    async def collect_jobs(self, params: Dict[str, Any], page_size: int = 150, max_offres: int | None = None) -> List[Dict[str, Any]]:
        """
    ...
    def _fetch_jobs_from_api(self, params: Dict[str, Any], page_size: int, max_offres: int | None) -> List[Dict[str, Any]]:
        jobs = self.job_client.collect_offres_paginated(params, page_size=page_size, max_offres=max_offres)
        self.logger.info("[CollectJobsService] Collecte terminée nombre d'offres = ", nb_offres=len(jobs))
        for job in jobs:
            job['source_id'] = job.get('id')
        return jobs

    async def _deactivate_missing_jobs(self, jobs: List[Dict[str, Any]], existing_jobs: List[str]) -> None:
    ...
    def _filter_existing_jobs(self, jobs: List[Dict[str, Any]], existing_jobs: List[str]) -> List[Dict[str, Any]]:
    ...
    async def _save_new_jobs(self, jobs: List[Dict[str, Any]]) -> None:
        await self.job_repo.save_jobs(jobs)
